...
function fetchBuildNumber() {
    ...
    if (typeof buildNumber === "string" && buildNumber.length > 0) {
        ...
        if (!BUILD_NUMBER_PATTERN.test(buildNumber)) {
            const message =
                "Invalid build number pattern: Must be a period separated list of at most three non-negative integers.";
...
async function setAppVersions() {
    ...
    configJson.elements[0].attributes["android-versionCode"] =
        androidVersionCode;
    console.log(
        colors.blue(`Set Android version code to ${androidVersionCode}`)
    );
