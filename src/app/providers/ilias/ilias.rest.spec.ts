...
describe("a ILIAS token manager", () => {
    ...
    let manager: ILIASTokenManager = new ILIASTokenManager(
        mockHttpClient,
        mockDataSupplier,
        mockResponseConsumer
    );
    ...
    describe("an access token to get", () => {
        describe("valid access token", () => {
            it("should return the access token", async () => {
                ...
                spyOn(mockDataSupplier, "getClientCredentials").and.returnValue(
                    Promise.resolve(clientCredentials)
                );
        ...
        describe("expired access token", () => {
            it("should get a new access token", async () => {
                ...
                >{
                    ...
                    token: <Token>{
                        ...
                        lastAccessTokenUpdate:
                            Date.now() / FACTOR_SEC_TO_MILLI - TWO_HOURS_IN_SEC,
                        accessTokenTTL: ONE_HOUR_IN_SEC,
                    },
                };
                spyOn(mockDataSupplier, "getClientCredentials").and.returnValue(
                ...
                const mockResponse: jasmine.SpyObj<HttpResponse> =
                    createSpyObject(HttpResponse);
                mockResponse.json.and.returnValue({
                ...
                const handleStub: jasmine.Spy =
                    mockResponse.handle.and.callFake((args) => {
                ...
                const consumeStub: jasmine.Spy = spyOn(
                    mockResponseConsumer,
                    "accept"
                );
        ...
        describe("thrown error from dependency", () => {
            it("should throw a token expired error", async () => {
                ...
                await expectAsync(
                    ...
                    TokenExpiredError,
                    "Could not get a valid access token"
                );
            });
        });
    });
