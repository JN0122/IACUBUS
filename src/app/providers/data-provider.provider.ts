...
export class DataProvider {
    ...
    private readonly runningDownloadTasks: Map<
        number,
        Promise<Array<ILIASObject>>
    > = new Map<number, Promise<Array<ILIASObject>>>();
    ...
    async getDesktopData(user: User): Promise<Array<ILIASObject>> {
        const localDesktopItems: Array<ILIASObject> =
        ...
        if (localDesktopItems.length === 0) {
            this.log.debug(
                () =>
                    "No local desktop data found wait for object metadata download"
            );
            const objects: Array<ILIASObject> =
                await this.loadRemoteDesktopData(user);
            ...
            if (!this.runningDownloadTasks.has(refId)) {
                this.log.debug(
                    () =>
                        "Local data desktop available refresh data in background task"
                );

                // Download task
                const task: Promise<Array<ILIASObject>> =
                    this.loadRemoteDesktopData(user);
                ...
                task.then(() => this.runningDownloadTasks.delete(refId))
                    .then(() =>
                        this.log.debug(
                            () =>
                                "Background task -> desktop object data fetch complete"
                        )
                    )
                    .catch((error) => {
                        ...
                        this.log.error(
                            () =>
                                `Background task -> desktop object data fetch failed with message: "${error.message}"`
                        );
                        throw error;
                    });
            } else {
                this.log.debug(
                    () =>
                        "Background task for desktop object data fetch already running"
                );
            }
     ...
     */
    async getObjectData(
        options: GetObjectDataOptions
    ): Promise<Array<ILIASObject>> {
        this.log.debug(() => "Fetch object data");
        // Fetch local data
        const localObjects: Array<ILIASObject> = options.recursive
            ? await ILIASObject.findByParentRefIdRecursive(
                  options.parentObject.refId,
                  options.user.id
              )
        ...
        if (options.downloadMetadata) {
            ...
            if (localObjects.length === 0) {
                this.log.debug(
                    () =>
                        "No local data found wait for object metadata download"
                );
                const objects: Array<ILIASObject> = await this.loadRemoteData(
                    options
                );
                return objects.sort(ILIASObject.compare);
            } else {
                // Schedule download and return local data, if task is not already running
                if (
                    !this.runningDownloadTasks.has(options.parentObject.refId)
                ) {
                    this.log.debug(
                        () =>
                            "Local data available refresh data in background task"
                    );

                    // Download task
                    const task: Promise<Array<ILIASObject>> =
                        this.loadRemoteData(options);

                    // Add running task to list because we dont want to execute it twice
                    this.runningDownloadTasks.set(
                        options.parentObject.refId,
                        task
                    );
                    ...
                    // Remove task after we are done
                    task.then(() =>
                        this.runningDownloadTasks.delete(
                            options.parentObject.refId
                        )
                    )
                        .then(() =>
                            this.log.debug(
                                () =>
                                    "Background task -> object data fetch complete"
                            )
                        )
                        .catch((error) => {
                            this.runningDownloadTasks.delete(
                            ...
                            this.log.error(
                                () =>
                                    `Background task -> object data fetch failed with message: "${error.message}"`
                            );
                            throw error;
                        });
                } else {
                    this.log.debug(
                        () =>
                            "Background task for object data fetch already running"
                    );
                }
    ...
    private async loadRemoteData(
        ...
        const data: Array<DesktopData> = await this.rest.getObjectData(
            options.parentObject.refId,
            options.user,
            options.recursive
        );
        return this.storeILIASObjects(
            data,
            options.user,
    ...
    private async loadRemoteDesktopData(
        user: User
    ): Promise<Array<ILIASObject>> {
     ...
     * @returns {Promise<ILIASObject>}
     */
    private async storeILIASObject(
        object: DesktopData,
        user: User,
        rootParent: ILIASObject | undefined = undefined,
        refreshFiles: boolean = true
    ): Promise<ILIASObject> {
        this.log.trace(
            () =>
                `Storing ILIAS Object with objId: ${object.objId} and refId: ${object.refId}`
        );

        const iliasObject: ILIASObject = await ILIASObject.findByRefIdAndUserId(
            parseInt(object.refId, 10),
            user.id
        );
    ...
    private async onSaveFile(
        user: User,
        iliasObject: ILIASObject
    ): Promise<ILIASObject> {
        const saveResult: ILIASObject = await this.fileObjectHandler.onSave(
            iliasObject,
            user
        );
     ...
     * @returns {Promise<ILIASObject[]>}
     */
    private async storeILIASDesktopObjects(
        objects: Array<DesktopData>,
        user: User
    ): Promise<Array<ILIASObject>> {
     ...
     * @returns {Promise<ILIASObject[]>}
     */
    private async storeILIASObjects(
        remoteObjects: Array<DesktopData>,
        user: User,
        parentObject: ILIASObject,
        ...
        if (recursive) {
            const existingObjects: Array<ILIASObject> =
                await ILIASObject.findByParentRefIdRecursive(
                    parentObject.refId,
                    user.id
                );
            return this.saveOrDeleteObjects(
                remoteObjects,
                existingObjects,
                user,
                parentObject,
     ...
     * @returns {Promise<ILIASObject[]>}
     */
    private async saveOrDeleteObjects(
        remoteObjects: Array<DesktopData>,
        existingObjects: Array<ILIASObject>,
        user: User,
        rootParent: ILIASObject,
        refreshFiles: boolean = true
    ): Promise<Array<ILIASObject>> {
        const iliasObjects: Array<ILIASObject> = [];
        const objectsToDelete: Array<ILIASObject> = existingObjects;
        this.log.trace(
            () => `Existing Objects: ${JSON.stringify(existingObjects)}`
        );

        for (const remoteObject of remoteObjects) {
            const iliasObject: ILIASObject = await this.storeILIASObject(
                remoteObject,
                user,
                rootParent,
                refreshFiles
            ...
            const objectIndex: number = existingObjects.findIndex(
                (existingObject) => {
