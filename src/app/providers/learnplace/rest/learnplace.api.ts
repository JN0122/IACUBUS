...
import {
    blocksJsonSchema,
    journalEntriesJsonSchema,
    learnplaceJsonSchema,
} from "./json.schema";
import {
    BlockObject,
    ILIASLinkBlock,
...
export const LEARNPLACE_API: InjectionToken<LearnplaceAPI> = new InjectionToken(
    "token for learnplace api"
);
...
export class ILIASLearnplaceAPI implements LearnplaceAPI {
    ...
    async getLearnPlace(objectId: number): Promise<LearnPlace> {
        const response: HttpResponse = await this.iliasRest.get(
            `/v2/ilias-app/learnplace/${objectId}`,
    ...
    async getBlocks(learnplaceObjectId: number): Promise<BlockObject> {
        const response: HttpResponse = await this.iliasRest.get(
            `/v2/ilias-app/learnplace/${learnplaceObjectId}/blocks`,
            DEFAULT_REQUEST_OPTIONS
        );
     ...
     * @private
     */
    private async downloadLinkBlockRelatedILIASObject(
        blocks: BlockObject
    ): Promise<void> {
        ...
        await this.ilObjService.downloadIlObjByRefID(
            linkBlocks.map((block) => block.refId)
        );
    }
}
