 ...
 */
@Injectable()
export class TypeORMMapRepository
    extends AbstractCRUDRepository<MapEntity, number>
    implements MapRepository
{
     ...
     * @throws {RepositoryError} if an error occurs during this operation
     */
    async findByLearnplaceId(
        learnplaceId: number
    ): Promise<Optional<MapEntity>> {
        try {
            ...
            const result: MapEntity | null = await this.connection
                ...
                .select()
                .from(MapEntity, "map")
                .where("map.FK_learnplace = :learnplaceId", {
            ...
            throw new RepositoryError(
                Logging.getMessage(
                    error,
                    `Could not find MapEntity by learnplaceId "${learnplaceId}"`
                )
            );
        }
    }
...
export const MAP_REPOSITORY: InjectionToken<MapRepository> = new InjectionToken(
    "token for TypeORM map repository"
);
