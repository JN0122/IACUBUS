...
import {
    ...
    FILE_DOWNLOADER,
    FileDownloader,
} from "./file-transfer/file-download";
import { HttpResponse } from "./http";
import { User } from "../models/user";
import { Settings } from "../models/settings";

const DEFAULT_OPTIONS: ILIASRequestOptions = <ILIASRequestOptions>{
...
export class ILIASRestProvider {
    ...
    async getAuthToken(user: User): Promise<string> {
        const response: HttpResponse = await this.iliasRest.get(
            "/v2/ilias-app/auth-token",
            DEFAULT_OPTIONS
        );

        return response.handle(
            (it) => it.json<AuthToken>(authTokenSchema).token
        );
    }

    async getDesktopData(user: User): Promise<Array<DesktopData>> {
        const response: HttpResponse = await this.iliasRest.get(
            "/v2/ilias-app/desktop",
            DEFAULT_OPTIONS
        );
    ...
    async getObjectData(
        ...
        timeout: number = 0
    ): Promise<Array<DesktopData>> {
        const opt: ILIASRequestOptions = recursive
            ? { accept: "application/json", urlParams: [["recursive", "1"]] }
            : DEFAULT_OPTIONS;

        const response: HttpResponse = await this.iliasRest.get(
            `/v2/ilias-app/objects/${parentRefId}`,
            opt
        );
    ...
    async getFileData(
        ...
        const response: HttpResponse = await this.iliasRest.get(
            `/v3/ilias-app/files/${refId}`,
            DEFAULT_OPTIONS
        );

        return response.handle((it) => it.json<FileData>(fileSchema));
    }
    ...
    async downloadFile(
        ...
        return this.file.getFile(
            await this.file.resolveDirectoryUrl(storageLocation),
            fileName,
            <Flags>{
                create: false,
                exclusive: false,
    ...
    async postLearningProgressDone(fileRefId: number): Promise<void> {
        ...
        return response.handle<void>((_) => {
            this.log.trace(
                () =>
                    `Successfully posted learning-progress-to-done for iliasRefId ${fileRefId}`
            );
        });
    }
    ...
    async getThemeData(user: User): Promise<ThemeData> {
        ...
        const opt: ILIASRequestOptions = {
            accept: "application/json",
            urlParams: [["timestamp", settings.themeTimestamp.toString()]],
        };
        const response: HttpResponse = await this.iliasRest.get(
            "/v3/ilias-app/theme",
            opt
        ...
        return response.handle((it) => it.json<ThemeData>(themeSchema));
    }
}
