...
export class IconProvider {
    ...
    private readonly src: Map<string, SafeResourceUrl> = new Map<
        string,
        SafeResourceUrl
    >();
    ...
    async getIconSrc(key: string): Promise<string | SafeUrl> {
        if (this.src.has(key)) return this.src.get(key);

        try {
            if (
                await this.filesystem.checkFile(
                    `${this.filesystem.applicationDirectory}/www/assets/icon/`,
                    `icon_${key}.svg`
                )
            )
            ...
            else return `assets/icon/icon_${this.defaultIconKey}.svg`;
        }
    }
        ...
        if (
            ...
            for (let i: number = 0; i < this.icons.length; i++) {
                ...
                const settings: Settings =
                    await AuthenticationProvider.getUser().settings;
                const fileName: string = `${settings.themeTimestamp}_${icon.loadedName}`;
                let src: string | SafeResourceUrl;
                try {
                    let fileSrc: string = (
                        await this.filesystem.resolveLocalFilesystemUrl(
                            `${path}${fileName}`
                        )
                    ).toURL();
                    fileSrc = this.webview.convertFileSrc(fileSrc);
                    src =
                        this.sanitizer.bypassSecurityTrustResourceUrl(fileSrc);
                    this.src.set(icon.key, src);
                } catch (e) {
                    this.log.warn(
                        () =>
                            `unable to load custom icon ${fileName} in ${path}, resulted in error ${e.message}`
                    );
                }
            }
        }
