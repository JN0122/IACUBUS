...
export class LearningModuleStorageUtilisation implements StorageUtilization {
    private readonly log: Logger = Logging.getLogger(
        "LearningModuleStorageUtilisation"
    );
    ...
    async getUsedStorage(objectId: number, userId: number): Promise<number> {
        try {
            const lmDirPath: string = await this.pathBuilder.getLmDirByObjId(
                objectId
            );
            const dir: DirectoryEntry =
                await this.fileSystem.resolveDirectoryUrl(lmDirPath);
            ...
            if (error.code === 1) {
                this.log.debug(
                    () =>
                        `Directory of learning module with objId: ${objectId}, of user: ${userId} does not exist assume size 0 Byte`
                );
                return 0;
            }
            ...
            this.log.error(
                () =>
                    `Failed to calculate size for learning module with objId: ${objectId}, for user: ${userId}, reason: ${error.message}`
            );
            throw error;
        }
    }
