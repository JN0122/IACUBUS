...
export const LEARNING_MODULE_LOADER: InjectionToken<LearningModuleLoader> =
    new InjectionToken("token for learning module loader");
...
export class RestLearningModuleLoader implements LearningModuleLoader {
    ...
    async load(objId: number): Promise<void> {
        ...
        const request: LearningModuleData = await this.getLearningModuleData(
            obj.refId
        );
        this.loadingService.set(0.2);
        ...
        const localTmpZipDir: string = await this.pathBuilder.dirInLocalLmDir(
            "tmp",
            true
        );
        ...
        const url: string = await this.linkBuilder
            .resource()
            .resource(request.zipFile)
            .build();

        // return if the module did not change and is already loaded
        const lmLoaded: boolean = await LearningModule.existsByObjIdAndUserId(
            objId,
            user.id
        );
        ...
        await this.fileStorage.moveAndReplaceDir(
            localTmpZipDir,
            request.zipDirName,
            localAllLmsDir,
            this.pathBuilder.lmDirName(objId)
        );
