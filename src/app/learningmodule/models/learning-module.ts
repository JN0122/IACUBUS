...
export class LearningModule extends ActiveRecord<LearningModule> {
    ...
    constructor(id: number = 0) {
        super(
            id,
            new SQLiteConnector("learning_modules", [
                "objId",
                "userId",
                "relativeStartFile",
                "timestamp",
    ...
    static async findByObjIdAndUserId(
        ...
        const response: any = await db.query(
            "SELECT * FROM learning_modules WHERE objId = ? AND userId = ?",
            [objId, userId]
        );
     ...
     */
    static async existsByObjIdAndUserId(
        objId: number,
        userId: number
    ): Promise<boolean> {
        const db: SQLiteDatabaseService =
     ...
     */
    async getLocalStartFileUrl(
        pathBuilder: LearningModulePathBuilder
    ): Promise<string> {
        const localLmDir: string = await pathBuilder.getLmDirByObjId(
            this.objId
        );
