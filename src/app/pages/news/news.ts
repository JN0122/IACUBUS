...
export class NewsPage {
    newsPresenters: ReplaySubject<
        Array<[NewsItemModel, ILIASObjectPresenter]>
    > = new ReplaySubject(1);
    ...
    private async executeNewsSync(): Promise<void> {
        try {
            ...
            this.footerToolbar.addJob(
                Job.Synchronize,
                this.translate.instant("synchronisation_in_progress")
            );
            ...
            this.log.error(
                () =>
                    `Error occured in sync implemented in news page. Error: ${error}`
            );
            this.footerToolbar.removeJob(Job.Synchronize);
    ...
    private async fetchPresenterByRefId(
        refId: number
    ): Promise<ILIASObjectPresenter> {
        const userId: number = AuthenticationProvider.getUser().id;
        const ilObj: ILIASObject = await ILIASObject.findByRefIdAndUserId(
            refId,
            userId
        );
    ...
    private async fetchPresenterNewsTuples(): Promise<
        Array<[NewsItemModel, ILIASObjectPresenter]>
    > {
        const news: Array<NewsItemModel> =
            await this.newsFeed.fetchAllForCurrentUser();
        const mappedNews: Array<[NewsItemModel, ILIASObjectPresenter]> = [];
        for (const newsItem of news) {
            mappedNews.push([
                newsItem,
                await this.fetchPresenterByRefId(newsItem.newsContext),
            ]);
        }
        mappedNews.sort(
            (
                a: [NewsItemModel, ILIASObjectPresenter],
                b: [NewsItemModel, ILIASObjectPresenter]
            ): number => {
