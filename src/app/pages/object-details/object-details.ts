...
export class ObjectDetailsPage {
    ...
    private loadObjectDetails(): void {
        this.ilObjPresenterFactory
            .instance(this.object)
            .details()
            .then((details) => {
                this.log.debug(
                    () => `Details are displayed: ${JSON.stringify(details)}`
                );
    ...
    private loadAvailableActions(): void {
        this.actions = [
            this.openInIliasActionFactory(
                ...
                this.linkBuilder.default().target(this.object.refId)
            ),
        ];
        ...
        if (this.object.type == "file") {
            this.file.existsFile(this.object).then(
                () => {
                    this.actions.push(
                        new OpenFileExternalAction(
                            this.translate.instant(
                                "actions.open_in_external_app"
                            ),
                            this.object,
                            this.file
                        )
                ...
                () => {
                    this.log.info(
                        () =>
                            "No file available: Remove and Open are not available."
                    );
                }
            );
        }
