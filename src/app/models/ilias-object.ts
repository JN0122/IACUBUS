...
export class ILIASObject extends ActiveRecord<ILIASObject> {
    ...
    get repoPath(): Array<string> | string {
        if (this._repoPath) {
            try {
                ...
                this.log.error(
                    () => `Could not get json from: ${this._repoPath}`
                );
                return [];
            }
        }
    ...
    set repoPath(path: Array<string> | string) {
        if (typeof path === "string") {
            ...
            throw new Error(
                "Please provide a string or a list of strings for repoPath in ilias-object.ts"
            );
        }
    }
    ...
    getRootParent(): Promise<ILIASObject> {
        return this.getParentsChain().then((chain) =>
            Promise.resolve(chain[0])
        );
    }
    ...
    getParentsChain(): Promise<Array<ILIASObject>> {
        return this.parent.then((parentObject) => {
            if (!parentObject) {
                ...
                return parentObject.getParentsChain().then((chain) => {
                    chain.push(this);
    ...
    getParentsTitleChain(): Promise<Array<string>> {
        return this.getParentsChain().then((items) =>
    ...
    get parent(): Promise<ILIASObject> {
        return new Promise((resolve, reject) => {
            ILIASObject.findByRefIdAndUserId(
                this.parentRefId,
                this.userId
            ).then((parentObject) => {
                if (parentObject.id) {
    ...
    static getFavoritesByUserId(
        ...
        return SQLiteDatabaseService.instance()
            .then((db) =>
                db.query(
                    `SELECT * FROM objects WHERE isFavorite ${
                        includeLoading ? "> 0" : "= 1"
                    } AND userId = ? ORDER BY title ASC`,
                    [userId]
                )
            )
            .then((response) => {
                ...
                for (let i = 0; i < (<any>response).rows.length; i++) {
                    favorites.push(
                        ILIASObject.find(
                            <number>(<any>response).rows.item(i).id
                        )
                    );
                }
    ...
    async removeFromFavorites(
        ...
        while (objectsStack.length) {
            ...
            const newObjects: Array<ILIASObject> =
                await ILIASObject.findByParentRefId(ilObj.refId, this.userId);
     ...
     * @returns {Promise<ILIASObject>}
     */
    static async findByRefIdAndUserId(
        refId: number,
        userId: number
    ): Promise<ILIASObject> {
        ...
        const response: any = await db.query(
            "SELECT * FROM objects WHERE refId = ? AND userId = ?;",
            [refId, userId]
        );
        if (response.rows.length == 0) {
            ...
            const object: ILIASObject = await ILIASObject.find(
                response.rows.item(0).id
            );

            // We destroy all overdue instances.
            for (let i: number = 1; i < response.rows.length; i++) {
                await (
                    await ILIASObject.find(response.rows.item(i).id)
                ).destroy();
            }
     ...
     * @returns {Promise<ILIASObject>}
     */
    static async findByObjIdAndUserId(
        refId: number,
        userId: number
    ): Promise<ILIASObject> {
        // Workaround sometimes the ids are strings instead of numbers ...
        ...
        const db: SQLiteDatabaseService =
            await SQLiteDatabaseService.instance();
        const response: any = await db.query(
            "SELECT * FROM objects WHERE objId = ? AND userId = ?",
            [refId, userId]
        );
        if (response.rows.length == 0) {
            const object: ILIASObject = new ILIASObject();
     ...
     * @returns {Promise<ILIASObject[]>}
     */
    static findByParentRefId(
        parentRefId: number,
        userId: number
    ): Promise<Array<ILIASObject>> {
        const sql: string =
            "SELECT * FROM objects WHERE parentRefId = ? AND userId = ?";
    ...
    static findNewObjects(userId: number): Promise<Array<ILIASObject>> {
        const sql: string =
            "SELECT * FROM objects WHERE userId = ? AND (isNew = ? OR isUpdated = ?)";
    ...
    static getAllOpenDownloads(user: User): Promise<Array<ILIASObject>> {
        const sql: string =
            "SELECT * FROM objects WHERE isFavorite = ? AND userId = ?";
     ...
     * @returns {Promise<ILIASObject[]>}
     */
    static async findByParentRefIdRecursive(
        parentRefId: number,
        userId: number
    ): Promise<Array<ILIASObject>> {
        // Workaround sometimes the ids are strings instead of numbers ...
        ...
        const children: Array<ILIASObject> =
            await ILIASObject.findByParentRefId(parentRefId, userId);
     ...
     */
    static async setOfflineAvailableRecursive(
     ...
     */
    async updateNeedsDownload(
        childNeedsUpdate = null
    ): Promise<Array<ILIASObject>> {
        this.log.debug(
            () =>
                `Recursive update needs download. going through: ${this.title}`
        );
        if (this.type === "file") {
            // A file needs to check its file state and then escalate.
            return FileData.find(this.id).then((fileData) => {
                return this.saveAndEscalateNeedsDownload(
                    fileData.needsDownload()
                );
            });
            ...
            return ILIASObject.findByParentRefId(this.refId, this.userId).then(
                (objects) => {
     ...
     */
    private async saveAndEscalateNeedsDownload(
        newValue: boolean
    ): Promise<Array<ILIASObject>> {
        if (newValue == this.needsDownload) {
            this.log.debug(
                () =>
                    `Needs download stays the same for "${this.title}", with objId: ${this.objId}. No need for escalation.`
            );
        ...
        if (parent) {
            const objects: Array<ILIASObject> =
                await parent.updateNeedsDownload(this.needsDownload);
            objects.push(this);
            return objects;
        } else {
            return Array.of(this);
