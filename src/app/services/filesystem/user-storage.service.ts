...
export class UserStorageService {
    ...
    async removeObject(iliasObject: ILIASObject): Promise<void> {
        ...
        if (iliasObject.isLearnplace())
            return this.learnplaceManager.remove(
                iliasObject.objId,
                iliasObject.userId
            );

        if (iliasObject.type === "htlm" || iliasObject.type === "sahs")
            return this.learningModuleManager.remove(
                iliasObject.objId,
                iliasObject.userId
            );
        ...
        try {
            ...
            const iliasObjects: Array<ILIASObject> =
                await ILIASObject.findByParentRefIdRecursive(
                    containerObject.refId,
                    containerObject.userId
                );
            ...
            this.log.error(
                () =>
                    `An error occurred while deleting recursive files: ${JSON.stringify(
                        error
                    )}`
            );
            throw error;
    ...
    async deleteAllCache(userId: number): Promise<void> {
        const ilObjList: Array<ILIASObject> = await ILIASObject.findByUserId(
            userId
        );
