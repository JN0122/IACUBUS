...
const DEFAULT_REQUEST_OPTIONS: ILIASRequestOptions = <ILIASRequestOptions>{
...
export class IliasObjectService {
    ...
    constructor(@Inject(ILIAS_REST) private readonly iliasRest: ILIASRest) {}

    async downloadIlObjByRefID(
        refIds: Array<number>
    ): Promise<Array<ILIASObject>> {
        try {
            const user: User = await User.currentUser();
            const pendingDownloads: Array<Promise<HttpResponse>> = new Array<
                Promise<HttpResponse>
            >();
            const objNotFoundLocally: Map<number, ILIASObject> = new Map<
                number,
                ILIASObject
            >();

            for (const refId of refIds) {
                const localObject: ILIASObject =
                    await ILIASObject.findByRefIdAndUserId(refId, user.id);
                if (localObject.id === 0) {
                    pendingDownloads.push(
                        this.iliasRest.get(
                            `/v3/ilias-app/object/${refId}`,
            ...
            const finishedDownloads: Array<HttpResponse> = await Promise.all(
                pendingDownloads
            );

            const iliasObjects: Array<DesktopData> = finishedDownloads.map(
                (response) =>
                    response.handle<DesktopData>((it) =>
                        it.json(iliasObjectJsonSchema)
                    )
            );
            const localObjects = [];
            for (const remoteObject of iliasObjects) {
                const localObject: ILIASObject = objNotFoundLocally.get(
                    parseInt(remoteObject.refId, 10)
                );
            ...
            this.log.warn(
                () =>
                    `Failed to download link block related ilias objects, with error: [error=unknown, message="${error.message}"]`
            );
