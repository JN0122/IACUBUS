...
export const BLOCK_SERVICE: InjectionToken<BlockService> =
    new InjectionToken<BlockService>("token for block service");
...
export class VisibilityManagedBlockService implements BlockService {
    private readonly log: Logger = Logging.getLogger(
        VisibilityManagedBlockService.name
    );
     ...
     */
    getBlockList(learnplaceObjectId: number): Observable<Array<BlockModel>> {
        const learnplaceEntity: Observable<LearnplaceEntity> = from(
        ...
        return learnplaceEntity.pipe(
            mergeMap((it) => {
                this.log.trace(() => `Map blocks of learnplace: id=${it.id}`);

                return combineLatest<Array<BlockModel>>(
                    Array.of<Observable<BlockModel>>(
    ...
    private mapTextblocks(
        textBlockList: Array<TextblockEntity>
    ): Array<Observable<TextBlockModel>> {
        return textBlockList.map((it) => {
            const model: TextBlockModel = new TextBlockModel(
                it.sequence,
                this.sanitizer.bypassSecurityTrustHtml(it.content)
            );
            return this.strategyApplier.apply(
                model,
                VisibilityStrategyType[it.visibility.value]
            );
        });
    }

    private mapPictureBlocks(
        pictureBlockList: Array<PictureBlockEntity>
    ): Array<Observable<PictureBlockModel>> {
        return pictureBlockList.map((it) => {
            const model: PictureBlockModel = new PictureBlockModel(
                it.sequence,
                it.title,
                it.description,
                it.thumbnail,
    ...
    private mapLinkBlocks(
        linkBlockList: Array<LinkblockEntity>
    ): Array<Observable<LinkBlockModel>> {
        return linkBlockList.map((it) => {
            const model: LinkBlockModel = new LinkBlockModel(
                it.sequence,
                it.refId
            );
            return this.strategyApplier.apply(
    ...
    private mapVideoBlocks(
        videoBlockList: Array<VideoBlockEntity>
    ): Array<Observable<VideoBlockModel>> {
        return videoBlockList.map((it) => {
            const model: VideoBlockModel = new VideoBlockModel(
                it.sequence,
                it.url
            );
            return this.strategyApplier.apply(
    ...
    private mapAccordionBlock(
        accordionBlockList: Array<AccordionEntity>
    ): Array<Observable<AccordionBlockModel>> {
        return accordionBlockList.map((accordion) => {
            const blockList: Observable<Array<BlockModel>> = combineLatest<
                Array<BlockModel>
            >([
            ...
            return this.strategyApplier.apply(
                model,
                VisibilityStrategyType[accordion.visibility.value]
            );
        });
    }
}
