...
describe("a learnplace loader", () => {
    ...
    let mockTextBlockMapper: jasmine.SpyObj<TextBlockMapper> =
        createSpyObject(TextBlockMapper);
    let mockPictureBlockMapper: jasmine.SpyObj<PictureBlockMapper> =
        createSpyObject(PictureBlockMapper);
    let mockLinkBlockMapper: jasmine.SpyObj<LinkBlockMapper> =
        createSpyObject(LinkBlockMapper);
    let mockVideoBlockMapper: jasmine.SpyObj<VideoBlockMapper> =
        createSpyObject(VideoBlockMapper);
    let mockAccordionMapper: jasmine.SpyObj<AccordionMapper> =
        createSpyObject(AccordionMapper);
    let mockJournalEntryMapper: jasmine.SpyObj<VisitJournalMapper> =
        createSpyObject(VisitJournalMapper);
    ...
    describe("a learnplace to load", () => {
        describe("on new learnplace", () => {
            it("should create the learnplace and its relations", async () => {
                ...
                spyOn(mockLearnplaceAPI, "getLearnPlace").and.returnValue(
                    Promise.resolve(learnplace)
                );

                const blocks: BlockObject = createEmptyBlocks(); // we don't care about the blocks in this class
                spyOn(mockLearnplaceAPI, "getBlocks").and.returnValue(
                ...
                const saveStub: jasmine.Spy = spyOn(
                    mockLearnplaceRepository,
                    "save"
                );
                saveStub.and.returnValue(
                    Promise.resolve(new LearnplaceEntity())
                ); // return value is not used, therefore an empty entity is enough

                // again, we don't care about the blocks or visit journal, therefore we return empty arrays
                const textBlockMapperStub: jasmine.Spy =
                    mockTextBlockMapper.map;
                textBlockMapperStub.and.returnValue(Promise.resolve([]));

                const pictureBlockMapperStub: jasmine.Spy =
                    mockPictureBlockMapper.map;
                pictureBlockMapperStub.and.returnValue(Promise.resolve([]));

                const linkBlockMapperStub: jasmine.Spy =
                    mockLinkBlockMapper.map;
                linkBlockMapperStub.and.returnValue(Promise.resolve([]));

                const videoBlockMapperStub: jasmine.Spy =
                    mockVideoBlockMapper.map;
                videoBlockMapperStub.and.returnValue(Promise.resolve([]));

                const visitJournalMapperStub: jasmine.Spy =
                    mockJournalEntryMapper.map;
                visitJournalMapperStub.and.returnValue(Promise.resolve([]));

                const accordionMapperStub: jasmine.Spy =
                    mockAccordionMapper.map;
        ...
        describe("on existing learnplace", () => {
            it("should update the existing learnplace and its relations", async () => {
                ...
                spyOn(
                    ...
                    Promise.resolve(Optional.of(getExistingLearnplace()))
                );
                ...
                const expected: LearnplaceEntity =
                    new LearnplaceEntity().applies<LearnplaceEntity>(
                        function (): void {
                            this.id = "";
                            this.objectId = learnplace.objectId;
                            this.user = Promise.resolve(testUser());
                            this.map = new MapEntity().applies(
                                function (): void {
                                    this.id = 1;
                                    this.visibility = getVisibilityEntity(
                                        learnplace.map.visibility
                                    );
                                    this.zoom = 10;
                                }
                            );
                            this.location = new LocationEntity().applies(
                                function (): void {
                                    this.id = 1;
                                    this.latitude =
        ...
        describe("on http request error", () => {
            it("it should throw an learnplace loading error", async () => {
                spyOn(mockLearnplaceAPI, "getLearnPlace").and.callFake(() =>
                    Promise.reject(new HttpRequestError(500, ""))
                );

                spyOn(mockLearnplaceAPI, "getBlocks").and.callFake(() =>
                    Promise.reject(new HttpRequestError(500, ""))
                );

                spyOn(mockLearnplaceAPI, "getJournalEntries").and.callFake(() =>
                    Promise.reject(new HttpRequestError(500, ""))
                );
                ...
                spyOn(mockLearnplaceRepository, "exists").and.returnValue(
                    Promise.resolve(false)
                );

                await expectAsync(loader.load(1)).toBeRejectedWithError(
                    LearnplaceLoadingError,
                    'Could not load learnplace with id "1" over http connection'
                );
            });
        });
