    ...
    describe("on mapping text blocks", () => {
        describe("on new text blocks", () => {
            it("should create new text block entities", async () => {
                ...
                const result: Array<TextblockEntity> = await mapper.map(
                    local,
                    remote
                );
...
describe("a picture block mapper", () => {
    let mockResourceTransfer: ResourceTransfer = <ResourceTransfer>{
        transfer: (): undefined => undefined,
    };
    let mockFile: jasmine.SpyObj<File> = createSpyObject(File);
    let mockPlatform: jasmine.SpyObj<Platform> = createSpyObject(Platform);
    let mockPathBuilder: jasmine.SpyObj<LearnplacePathBuilder> =
        createSpyObject(LearnplacePathBuilderImpl);

    let mapper: PictureBlockMapper = new PictureBlockMapper(
        mockResourceTransfer,
        mockPathBuilder,
        mockFile
    );

    beforeEach(() => {
        mockResourceTransfer = <ResourceTransfer>{
            transfer: (): undefined => undefined,
        };
        mockFile = createSpyObject(File);
        mockPlatform = createSpyObject(Platform);
        mockPathBuilder = createSpyObject(LearnplacePathBuilderImpl);
    ...
    describe("on mapping picture blocks", () => {
        describe("on new picture blocks", () => {
            it("should create new picture block entities and transfer its pictures", async () => {
                ...
                spyOn(mockResourceTransfer, "transfer").and.returnValue(
                    Promise.resolve("absolute/local/path/image.png")
                );

                const result: Array<PictureBlockEntity> = await mapper.map(
                    local,
                    remote
                );
    ...
    describe("mapping link blocks", () => {
        describe("on new link blocks", () => {
            it("should create link block entities", async () => {
                ...
                const result: Array<LinkblockEntity> = await mapper.map(
                    local,
                    remote
                );
    ...
    describe("on mapping video blocks", () => {
        describe("on new video blocks", () => {
            it("should create new video block entities", async () => {
                ...
                spyOn(mockResourceTransfer, "transfer").and.returnValues(
                    Promise.resolve("absolute/path/image.png"),
                    Promise.resolve("absolute/path/image2.png")
                );

                const result: Array<VideoBlockEntity> = await mapper.map(
                    local,
                    remote
                );
                ...
                const expected: Array<VideoBlockEntity> = [
                    ...
                    new VideoBlockEntity().applies(function (): void {
                        this.iliasId = 2;
                        this.sequence = 2;
                        this.url = "absolute/path/image2.png";
                        this.hash = "4B8A";
                        this.visibility = getVisibilityEntity("NEVER");
                    }),
                ];
    ...
    describe("on mapping journal entries", () => {
        describe("on new journal entries", () => {
            it("should create new journal entities", async () => {
                ...
                const result: Array<VisitJournalEntity> = await mapper.map(
                    local,
                    remote
                );
...
describe("a accordion mapper", () => {
    let mockTextMapper: jasmine.SpyObj<TextBlockMapper> =
        createSpyObject(TextBlockMapper);
    let mockPictureMapper: jasmine.SpyObj<PictureBlockMapper> =
        createSpyObject(PictureBlockMapper);
    let mockLinkMapper: jasmine.SpyObj<LinkBlockMapper> =
        createSpyObject(LinkBlockMapper);
    let mockVideoMapper: jasmine.SpyObj<VideoBlockMapper> =
        createSpyObject(VideoBlockMapper);

    let mapper: AccordionMapper = new AccordionMapper(
        mockTextMapper,
        mockPictureMapper,
        mockLinkMapper,
        mockVideoMapper
    ...
    beforeEach(() => {
        mockTextMapper = createSpyObject(TextBlockMapper);
        mockPictureMapper = createSpyObject(PictureBlockMapper);
        mockLinkMapper = createSpyObject(LinkBlockMapper);
        mockVideoMapper = createSpyObject(VideoBlockMapper);
        mapper = new AccordionMapper(
            mockTextMapper,
            mockPictureMapper,
            mockLinkMapper,
            mockVideoMapper
    ...
    describe("mapping accordion blocks", () => {
        describe("on new accordion blocks", () => {
            it("should create new accordion entities", async () => {
                ...
                const result: Array<AccordionEntity> = await mapper.map(
                    local,
                    remote
                );
