...
describe("a visibility strategy applier", () => {
    const mockAlwaysStrategy: jasmine.SpyObj<AlwaysStrategy> =
        createSpyObject(AlwaysStrategy);
    const mockNeverStrategy: jasmine.SpyObj<NeverStrategy> =
        createSpyObject(NeverStrategy);
    const mockOnlyAtPlaceStrategy: jasmine.SpyObj<OnlyAtPlaceStrategy> =
        createSpyObject(OnlyAtPlaceStrategy);
    const mockAfterVisitPlaceStrategy: jasmine.SpyObj<AfterVisitPlaceStrategy> =
        createSpyObject(AfterVisitPlaceStrategy);

    let applier: VisibilityStrategyApplier = new VisibilityStrategyApplier(
        mockAlwaysStrategy,
        mockNeverStrategy,
        mockOnlyAtPlaceStrategy,
        mockAfterVisitPlaceStrategy
    );
        ...
        describe("strategy which requires a learnplace", () => {
            it("should throw an illegal state error", () => {
                ...
                expect(() =>
                    applier.apply(
                        mockModel,
                        VisibilityStrategyType.ONLY_AT_PLACE
                    )
                ).toThrowError(
                    IllegalStateError,
                    "Can not apply strategy without learnplace id: Call VisibilityStrategyApplier#setLearnplace first"
                );
            });
        });
