...
export class IliasCoordinates {
    ...
    constructor(readonly latitude: number, readonly longitude: number) {
        if (latitude < MIN_LAT || latitude > MAX_LAT) {
            throw new IllegalArgumentError(
                `Value for latitude is invalid: latitude=${latitude}`
            );
        }

        if (longitude < MIN_LONG || longitude > MAX_LONG) {
            throw new IllegalArgumentError(
                `Value for longitude is invalid: longitude=${longitude}`
            );
        }
    }
    ...
    distanceTo(point: IliasCoordinates): number {
        const [fromLat, fromLong, toLat, toLong]: Array<number> = [
            this.latitude,
            this.longitude,
            point.latitude,
            point.longitude,
        ...
        return round(
            EARTH_RADIUS *
                ...
                asin(
                    sqrt(
                        pow(sin(dLat / 2), 2) +
                            pow(sin(dLong / 2), 2) * cos(fromLat) * cos(toLat)
                    )
                ) *
                1000
        );
