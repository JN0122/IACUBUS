...
describe("a visit journal synchronization", () => {
    ...
    const mockVisitJournalRepository: VisitJournalRepository = <
    ...
    let sync: VisitJournalSynchronizationImpl =
        new VisitJournalSynchronizationImpl(
            mockLearnplaceAPI,
            mockVisitJournalRepository
        );

    beforeEach(() => {
        sync = new VisitJournalSynchronizationImpl(
            mockLearnplaceAPI,
            mockVisitJournalRepository
        );
    });
    ...
    describe("a synchronization", () => {
        describe("on successful synchronization", () => {
            it("should save synchronized visit journals as synchronized", async () => {
                ...
                const apiStub: jasmine.Spy = spyOn(
                    mockLearnplaceAPI,
                    "addJournalEntry"
                ...
                const expected: VisitJournalEntity =
                    new VisitJournalEntity().applies(function (): void {
...
describe("a visit journal watch", () => {
    ...
    const mockLearnplaceRepository: LearnplaceRepository = <
    ...
    const mockGeolocation: jasmine.SpyObj<Geolocation> =
        createSpyObject(Geolocation);

    let watch: SynchronizedVisitJournalWatch =
        new SynchronizedVisitJournalWatch(
            mockLearnplaceAPI,
    ...
    beforeEach(() => {
        watch = new SynchronizedVisitJournalWatch(
            mockLearnplaceAPI,
            mockLearnplaceRepository,
            mockUserRepository,
            mockGeolocation
    ...
    describe("starting the watch", () => {
        describe("on no set learnplace id", () => {
            it("should throw an illegal state error", () => {
                expect(() => watch.start()).toThrowError(
                    IllegalStateError,
                    "Can not start SynchronizedVisitJournalWatch without learnplace id"
                );
            });
        });
    });
