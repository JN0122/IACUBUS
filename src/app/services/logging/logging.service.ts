...
import { hasOwnProperty } from "../../util/util.function";
...
export namespace Logging {
    ...
    export function shutdown(): void {
        appenders.forEach((it) => it.shutdown());
    }
...
export class DefaultLogger implements Logger {
    ...
    trace(msg: () => string): void {
        this.appenders.forEach((it) =>
            it.log(this.createEntry(LogLevel.TRACE, msg))
        );
    }

    debug(msg: () => string): void {
        this.appenders.forEach((it) =>
            it.log(this.createEntry(LogLevel.DEBUG, msg))
        );
    }

    info(msg: () => string): void {
        this.appenders.forEach((it) =>
            it.log(this.createEntry(LogLevel.INFO, msg))
        );
    }

    warn(msg: () => string): void {
        this.appenders.forEach((it) =>
            it.log(this.createEntry(LogLevel.WARN, msg))
        );
    }

    error(msg: () => string): void {
        this.appenders.forEach((it) =>
            it.log(this.createEntry(LogLevel.ERROR, msg))
        );
    }

    fatal(msg: () => string): void {
        this.appenders.forEach((it) =>
            it.log(this.createEntry(LogLevel.FATAL, msg))
        );
    }
