...
export const RESOURCE_LINK_BUILDER: InjectionToken<() => ResourceLinkBuilder> =
    new InjectionToken("token for the resource link builder factory");
        ...
        const user: UserEntity = (
            ...
            () =>
                new NoSuchElementError(
                    "No authenticated user found, unable to build resource ILIAS link."
                )
        );
    ...
    private validateBuilderState(): void {
        if (this.path === "" || isString(this.path) !== true)
            throw new IllegalStateError(
                "Required resource path was not found, ILIAS resource link build failed."
            );
    }
}
