...
describe("A resource link builder ", () => {
    ...
    let subject: ResourceLinkBuilder = new ResourceLinkBuilderImpl(
    ...
    beforeEach(() => {
        subject = new ResourceLinkBuilderImpl(
            mockInstallationLinkSupplier,
            mockTokenSupplier,
            mockUserRepository
        );
        ...
        describe("with an authenticated user and a valid resource path", () => {
            it("should build the ILIAS resource link.", async () => {
                ...
                await expectAsync(subject.build()).toBeResolvedTo(
                    `${installation}/${path}?user=${user.iliasUserId}&token=${token}&target=ilias_app_resource`
                );
            });
        });
    });
