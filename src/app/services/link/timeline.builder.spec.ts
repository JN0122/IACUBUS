...
describe("A timeline link builder ", () => {
    ...
    let subject: TimelineLinkBuilder = new TimelineLinkBuilderImpl(
    ...
    beforeEach(() => {
        subject = new TimelineLinkBuilderImpl(
            mockInstallationLinkSupplier,
            mockTokenSupplier,
            mockUserRepository
        );
        ...
        describe("with an authenticated user and a valid ref id", () => {
            it("should build the ILIAS timeline link.", async () => {
                ...
                await expectAsync(subject.build()).toBeResolvedTo(
                    `${installation}/goto.php?target=ilias_app_auth|${user.iliasUserId}|${refId}|timeline|${token}`
                );
            });
        });
    });
