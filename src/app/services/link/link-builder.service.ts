...
export const LINK_BUILDER: InjectionToken<LinkBuilder> = new InjectionToken(
    "token for loading link builder"
);
...
export class LinkBuilderImpl implements LinkBuilder {
    constructor(
        @Inject(DEFAULT_LINK_BUILDER)
        private readonly defaultLinkBuilder: () => DefaultLinkBuilder,
        @Inject(TIMELINE_LINK_BUILDER)
        private readonly timelineLinkBuilder: () => TimelineLinkBuilder,
        @Inject(NEWS_LINK_BUILDER)
        private readonly newsLinkBuilder: () => NewsLinkBuilder,
        @Inject(LOGIN_LINK_BUILDER)
        private readonly loginLinkBuilder: () => LoginLinkBuilder,
        @Inject(LOADING_LINK_BUILDER)
        private readonly loadingLinkBuilder: () => LoadingLinkBuilder,
        @Inject(RESOURCE_LINK_BUILDER)
        private readonly resourceLinkBuilder: () => ResourceLinkBuilder
    ) {}
