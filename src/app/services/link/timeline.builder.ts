...
export const TIMELINE_LINK_BUILDER: InjectionToken<() => TimelineLinkBuilder> =
    new InjectionToken("token for timeline link builder factory");
        ...
        const user: UserEntity = (
            ...
            () =>
                new NoSuchElementError(
                    "No authenticated user found, unable to build timeline ILIAS link."
                )
        );
    ...
    private validateBuilderState(): void {
        if (this.refId <= 0)
            throw new IllegalStateError(
                "Required ref id was not found, ILIAS timeline link build failed."
            );
    }
}
