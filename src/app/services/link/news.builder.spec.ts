...
describe("A news link builder ", () => {
    ...
    const mockUserRepository: UserRepository = <UserRepository>{
        findAuthenticatedUser: async (): Promise<Optional<UserEntity>> =>
            undefined,
    };

    let subject: NewsLinkBuilder = new NewsLinkBuilderImpl(
    ...
    beforeEach(() => {
        subject = new NewsLinkBuilderImpl(
            mockInstallationLinkSupplier,
            mockTokenSupplier,
            mockUserRepository
        );
        ...
        describe("with an authenticated user and a valid news id and context", () => {
            it("should build the ILIAS default object link.", async () => {
                ...
                await expectAsync(subject.build()).toBeResolvedTo(
                    `${installation}/goto.php?target=ilias_app_news|${user.iliasUserId}|${id}|${context}|${token}`
                );
            });
        });
    });
