...
export const NEWS_LINK_BUILDER: InjectionToken<() => NewsLinkBuilder> =
    new InjectionToken<() => NewsLinkBuilder>(
        "token for news link builder factory"
    );
...
export class NewsLinkBuilderImpl implements NewsLinkBuilder {
    ...
    constructor(
        @Inject(INSTALLATION_LINK_PROVIDER)
        private readonly installationLinkSupplier: InstallationLinkSupplier,
        ...
        const user: UserEntity = (
            ...
            () =>
                new NoSuchElementError(
                    "No authenticated user found, unable to build news ILIAS link."
                )
        );
    ...
    private validateBuilderState(): void {
        if (this.id <= 0)
            throw new IllegalStateError(
                "Required news id was not found, ILIAS news link build failed."
            );
        if (this.contextId <= 0)
            throw new IllegalStateError(
                "Required news context id was not found, ILIAS news link build failed."
            );
    }
}
