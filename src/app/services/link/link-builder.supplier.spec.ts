...
describe("An installation link supplier", () => {
    ...
    let subject: InstallationLinkSupplier = new InstallationLinkSupplierImpl(
        mockConfigProvider,
        mockUserRepository
    );

    beforeEach(() => {
        subject = new InstallationLinkSupplierImpl(
            mockConfigProvider,
            mockUserRepository
        );
    });
    ...
    describe("getting the installation url", () => {
        describe("without an authenticated user", () => {
            it("should throw a corresponding error.", async () => {
                ...
                await expectAsync(subject.get()).toBeRejectedWithError(
                    NoSuchElementError,
        ...
        describe("without a valid installation id", () => {
            it("should throw a corresponding error.", async () => {
                ...
                spyOn(mockConfigProvider, "loadInstallation").and.returnValue(
                    Promise.reject(new ReferenceError())
                );
        ...
        describe("with authenticated user and installation", () => {
            it("should return the current installation url.", async () => {
                ...
                spyOn(mockConfigProvider, "loadInstallation").and.returnValue(
                    Promise.resolve(Optional.of(installation))
                );

                await expectAsync(subject.get()).toBeResolvedTo(
                    installation.url
                );
            });
        });
