...
export class OpenObjectInILIASAction extends ILIASObjectAction {
    ...
    async execute(): Promise<ILIASObjectActionResult> {
        ...
        if (this.platform.is("android"))
            this.openUserDialog(() => this.openBrowserAndroid(ilasLink));
        else if (this.platform.is("ios"))
            this.openUserDialog(() => this.openBrowserIos(ilasLink));
        else
            throw new IllegalStateError(
                "Unsupported platform, unable to open browser for unsupported platform."
            );
    ...
    private async openBrowserIos(link: string): Promise<void> {
        ...
        const sfViewControllerAvailable: boolean =
        ...
        if (sfViewControllerAvailable) {
            ...
            this.safariViewController
                .show({
                    ...
                    tintColor: getComputedStyle(document.body).getPropertyValue(
                ...
                .subscribe(
                    ...
                    (error: unknown) =>
                        console.error(
                            () =>
                                `Encountered error while opening SFViewController: ${JSON.stringify(
                                    error
                                )}`
                        )
                );
...
export const OPEN_OBJECT_IN_ILIAS_ACTION_FACTORY: InjectionToken<
    (
        title: string,
...
>("token for open in ILIAS action");
