    ...
    async execute(): Promise<ILIASObjectActionResult> {
        try {
            ...
            return new ILIASObjectActionSuccess(
                this.translation.instant("actions.removed_local_learnplace")
            );
        } catch (error) {
            if (error instanceof IOError)
                return new ILIASObjectActionError(
                    this.translation.instant(
                        "actions.removed_local_learnplace_failed"
                    )
                );
...
export interface RemoveLocalLearnplaceActionFunction {
    (
        title: string,
        objectId: number,
        userId: number
